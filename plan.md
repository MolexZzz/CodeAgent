...
- [x] `MODIFY`：进入现有的仓库任务闭环（完整 Runtime 闭环仍在完善）。
...
- [x] 如果存在未解决错误、用户拒绝的关键工具调用或环境阻塞，不能进入 `DONE`，只能进入暂停/报告状态。
...
- [x] T1：解释问题 → `ANSWER`，无修改。
- [x] T2：重构建议 → `PLAN`，无修改。
- [x] T3：修复 bug → `MODIFY`。
- [x] T4：PLAN 阶段调用编辑工具 → `DENY`。
- [x] T5：重复读取同一范围 → 触发无进展保护。
- [x] T6：达到步数预算 → 暂停并询问继续。
- [x] T7：测试断言失败 → 回到实现。
- [x] T8：测试环境错误 → 停止盲目修改并报告。
- [x] T9：Ctrl-C → 中断并持久化状态。
- [x] T10：修改完成 → 必须经过 diff、test、verify。

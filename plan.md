...
- [ ] 定义三种任务模式：`ANSWER`、`PLAN`、`MODIFY`。
- [ ] `ANSWER`：允许读取、搜索、摘要和必要的只读命令，直接回答，不修改文件、不要求修改确认。
- [ ] `PLAN`：允许只读探索，输出具体实施方案、影响文件、风险和测试建议，绝不修改文件。
- [ ] `MODIFY`：进入完整的仓库任务闭环。
- [ ] 显式修改词优先：修复、修改、实现、添加、删除、重构、补测试、改代码等必须进入 `MODIFY`。
- [ ] 解释、分析、比较、为什么、是什么、是否对齐等问题默认进入 `ANSWER`。
- [ ] “怎么改、如何重构、给方案”等问题默认进入 `PLAN`。
...
| 2026-08-28 | 补充模块职责边界、`CompletionGuard` 和 P0 实际开发顺序 | 已完成 | 待提交 | `git diff --check` 通过 |

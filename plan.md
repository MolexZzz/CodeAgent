...
- [/] `ANSWER`：允许读取、搜索、摘要和必要的只读命令，直接回答，不修改文件、不要求修改确认。
- [/] `PLAN`：允许只读探索，输出具体实施方案、影响文件、风险和测试建议，绝不修改文件。
...
- [ ] 低置信度时向用户澄清“只分析还是直接修改”，不能让模型自行猜测。
...
| 2026-08-28 | 实现 P0.1 IntentRouter 初版：`ANSWER`/`PLAN` 只读分流，`MODIFY` 进入现有修改闭环 | 已完成 | 待提交 | `python -m py_compile ...` 通过；`pytest -q tests/test_agent_workflow.py tests/test_tools.py tests/test_retry_and_verification.py tests/test_context_manager.py tests/test_workspace.py`：31 passed |
